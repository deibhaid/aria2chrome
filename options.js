...
const NATIVE_HOST_SH_EMBED = "#!/bin/bash\n# Wrapper so Chrome Native Messaging \"path\" is a single executable (see README).\n#\n# Chrome starts native hosts with a minimal PATH. A shebang of #!/usr/bin/env python3\n# on the .py file then resolves to /usr/bin/python3 — not Homebrew's python3.13 — so\n# macOS Full Disk Access on the interpreter you use in Terminal does not apply.\n# Call python3 explicitly after restoring a normal PATH.\n#\n# Optional: put the absolute path to ONE Python binary in aria2chrome_python.txt\n# (same folder as this script) if Terminal's `python3 -c \"import sys; print(sys.executable)\"`\n# shows a venv path — Chrome does not load your shell's venv.\nDIR=\"$(cd \"$(dirname \"$0\")\" && pwd)\"\nexport PATH=\"/opt/homebrew/bin:/opt/homebrew/sbin:/usr/local/bin:/usr/local/sbin:/usr/bin:/bin:/usr/sbin:/sbin\"\nPIN=\"$DIR/aria2chrome_python.txt\"\nif [[ -f \"$PIN\" ]]; then\n  P=\"$(head -n 1 \"$PIN\" | tr -d '\\r' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//')\"\n  if [[ -n \"$P\" && -x \"$P\" ]]; then\n    exec \"$P\" \"$DIR/aria2chrome_native_host.py\"\n  fi\nfi\nif [[ -n \"${ARIA2CHROME_PYTHON:-}\" && -x \"${ARIA2CHROME_PYTHON}\" ]]; then\n  exec \"${ARIA2CHROME_PYTHON}\" \"$DIR/aria2chrome_native_host.py\"\nfi\nexec python3 \"$DIR/aria2chrome_native_host.py\"\n";
...
/** Base64 for UTF-8 strings. `btoa` only accepts Latin1; embedded Python may contain Unicode. */
function utf8ToBase64(str) {
  ...
  try {
    return typeof btoa === 'function' ? btoa(unescape(encodeURIComponent(str))) : '';
  } catch (e) {
    return '';
  }
}
...
function buildNativeHostBashWritePayloadLines() {
  const b64Py = utf8ToBase64(NATIVE_HOST_PY_EMBED);
  const b64Sh = utf8ToBase64(NATIVE_HOST_SH_EMBED);
  return [
